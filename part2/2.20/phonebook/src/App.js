...
const Notification = ({ message }) => message ? <div className="notification">{message}</div> : null
const ErrorMessage = ({ errorMessage }) => errorMessage ? <div className="error">{errorMessage}</div> : null
...
const App = () => {
  ...
  const addPerson = (event) => {
    ...
    if (changedPerson) {
      ...
      .catch((e)=> {
        const errMsg = `Error in adding ${personObject.name}`
        console.log(errMsg);
        setErrorMessage(errMsg);
        setTimeout(() => {
          setErrorMessage(null);
